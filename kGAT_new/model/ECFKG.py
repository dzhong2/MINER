...
class ECFKG(nn.Module):
    ...
    def calc_score(self, user_ids, item_ids, relation_id):
        ...
        if self.random_embedding > 0:
            ...
            item_embed = F.normalize(item_embed + sampler.sample(item_embed.shape)[:, :, 0].to(item_embed.device))
        if self.random_rec > 0:
            num_shuffle = int(self.random_rec * item_embed.shape[0])
            shuffle_inds = torch.randperm(item_embed.shape[0])[: num_shuffle]
            item_embed[shuffle_inds] = item_embed[shuffle_inds[torch.randperm(num_shuffle)]]
