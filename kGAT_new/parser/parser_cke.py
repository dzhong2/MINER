...
def parse_cke_args():
    ...
    parser.add_argument('--get_loss', type=int, default=0,
                        help='if not zero, save loss on testing')
    parser.add_argument('--rm', type=int, default=0,
                        help='0-99, remove the interaction if specified')

    parser.add_argument('--get_score', type=int, default=0,
                        help='if not zero, save score on testing')
    ...
    if args.train_shadow:
        if args.kg_partial == 0:
            ...
            shadow_sub_name = "shadow({})".format(int(args.partial_option) - 3 + 2 + args.kg_partial) if args.partial_option else "shadow"
        save_dir = 'trained_model/CKE/{}/{}_embed-dim{}_relation-dim{}_lr{}_pretrain{}/'.format(
            args.data_name, shadow_sub_name, args.embed_dim, args.relation_dim, args.lr, args.use_pretrain)
        if args.kg_only:
            save_dir = save_dir.replace("/shadow", f"/shadow-kg")
    else:
        save_dir = 'trained_model/CKE/{}/{}_embed-dim{}_relation-dim{}_lr{}_pretrain{}/'.format(
            args.data_name, 'target', args.embed_dim, args.relation_dim, args.lr, args.use_pretrain)
        if args.www > 0:
            save_dir = save_dir.replace("target", f"WWW_{args.www}_target")
        if args.rm > 0:
            save_dir = save_dir.replace("target", f"target_RM_{args.rm}")
